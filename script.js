...
function calcularEconomiaGNVAtual() {
  ...
  const resultado = `
    ...
    <ul>
      ...
      ${ipvaGasolina > 0 ? `<li>🏷️ Economia apenas com IPVA em 5 anos: <strong>R$ ${(ipvaGasolina - ipvaGNV).toFixed(2)}</strong></li>` : ""}
...
function calcularEconomia() {
  ...
  document.getElementById("resultadoCenario1").innerHTML = `
    ...
    <p><strong>Economia mensal GNV vs Gasolina:</strong> R$ ${(gastoGasolina - gastoGNV).toFixed(2)}</p>
    <p><strong>Economia mensal GNV vs Etanol:</strong> R$ ${(gastoEtanol - gastoGNV).toFixed(2)}</p>
    ${mensagemRetorno}
